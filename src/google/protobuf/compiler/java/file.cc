...
void FileGenerator::Generate(io::Printer* printer) {
  ...
  for (int i = 0; i < file_->enum_type_count(); i++) {
    if (IsOwnFile(file_->enum_type(i), immutable_api_)) continue;
    generator_factory_->NewEnumGenerator(file_->enum_type(i))
        ->Generate(printer);
  }
  for (int i = 0; i < file_->message_type_count(); i++) {
    if (IsOwnFile(file_->message_type(i), immutable_api_)) continue;
    message_generators_[i]->GenerateInterface(printer);
    message_generators_[i]->Generate(printer);
  }
  if (HasGenericServices(file_, context_->EnforceLite())) {
    for (int i = 0; i < file_->service_count(); i++) {
      if (IsOwnFile(file_->service(i), immutable_api_)) continue;
...
void FileGenerator::GenerateSiblings(
  ...
  for (int i = 0; i < file_->message_type_count(); i++) {
    ...
    if (!IsOwnFile(message_type, immutable_api_)) continue;
    std::string java_package =
  ...
  if (HasGenericServices(file_, context_->EnforceLite())) {
    for (int i = 0; i < file_->service_count(); i++) {
      ...
      if (!IsOwnFile(service, immutable_api_)) continue;
      std::unique_ptr<ServiceGenerator> generator(
